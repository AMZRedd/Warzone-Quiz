...
import random
names = []
global warzone_questions_answers
asked=[]
score=0


warzone_questions_answers = {
 1: ["What was the date that Warzone had offically been released?",'October 1982','April 2022', 'February 2018','March 2020','March 2020',4],   
  2: ["What nerf was made to make the DMR viable?", 'Decreased vertical recoil control','Decreased horizontal recoil control', 'TTK Headshot damage from 1.2 to 1.0', 'All of them', 'Decreased vertical recoil control',1], 
  3: ["Which perk will increase you revive speed by 25%?", "Pointman", 'Tuneup', "Quick Fix", "Overkill", 'Tuneup',2], 
  4: ["How many bunkers are in the game?", '4', "11", "16", '3',"11", 2], 5: ["Who is the best Warzone player in the world?", 'AMZ_Redd', "Nick Mercs", "Aydan", "Joewoe", "Aydan",3], 
  6: ["How many GUNS are there in Warzone?", '30',"98", "77", '53', "77",3], 
  7: ["Where is the location of Storage Town?", 'Northern part', "Southern part", "Eastern Part", 'Western part', 'Western part',4], 
  8: ["What melee weapon gives you a 1.5% speed increase ?", 'Kali sticks', "Combat knife", "E-Tool", 'Dual Kodachis','Kali sticks',1], 
  9: ["What optic do you use to get the Clean living reticle ?",'VLK optic', "Corp Holo Combat",'Axial arms 3x', 'Microflex LED', 'Axial arms 3x',3], 
  10: ["What weapon is not a sniper rifle?", 'CR5.56 AMAX', 'AX-50', 'SwissK31', 'Pelington 703','CR5.56 AMAX',1],
 } 
def randomiser():
  global qnum 
    ...
    def __init__(self, parent):
      ...
      # Create a Button
        ...
        self.continue_button = Button(self.quiz_frame, text='Continue',
                bg='lime', command=self.name_collection)
...
class Quiz:        
    def __init__(self, parent):
      ...
      self.question_label=Label(self.quiz_frame, text= warzone_questions_answers[qnum][0], fg = "white",
      bg = background_color,
      ...
      self.rb1 = Radiobutton (self.quiz_frame, text = warzone_questions_answers[qnum][1], font=("Helvetica", "12", "bold"),fg="Black", bg="lime", value=1, variable=self.var1, pady=10)
      self.rb1.grid(row=1, pady=20) 
  
      self.rb2 = Radiobutton (self.quiz_frame, text = warzone_questions_answers[qnum][2], font=("Helvetica", "12", "bold"),fg="Black", bg="lime", value=2, variable=self.var1, pady=10)
      self.rb2.grid(row=2, pady=20) 

      self.rb3 = Radiobutton (self.quiz_frame, text = warzone_questions_answers[qnum][3], font=("Helvetica", "12", "bold"),fg="Black", bg="lime", value=3, variable=self.var1, pady=10)
      self.rb3.grid(row=3, pady=20) 

      self.rb4 = Radiobutton (self.quiz_frame, text = warzone_questions_answers[qnum][4], font=("Helvetica", "12", "bold"),fg="Black", bg="lime", value=4, variable=self.var1, pady=10)
      self.rb4.grid(row=4, pady=20) 

      
      self.Submit_button = Button(self.quiz_frame, text="Submit", font=("Helvetica","13","bold"), bg="lime", command=self.test_progress)
    ...
    def questions_setup(self):
      randomiser()
      self.var1.set(0)
      self.question_label.config(text=warzone_questions_answers[qnum][0])
      self.rb1.config(text=warzone_questions_answers[qnum][1])
      self.rb2.config(text=warzone_questions_answers[qnum][2]) 
      self.rb3.config(text=warzone_questions_answers[qnum][3])
      self.rb4.config(text=warzone_questions_answers[qnum][4])
    ...
    def test_progress(self):
        ...
        if len(asked)>9:#if it is the last question
            if choice==warzone_questions_answers[qnum][6]:#if last question is right answer
              score +=1
              scr_label.configure(text=score)
              self.Submit_button.config(text="Submit")
            else: #is last question is wrong answer
                score+=0
                scr_label.config(text="The correct answer was " + warzone_questions_answers[qnum][5])
                self.Submit_button.config(text="Submit")
            ...
            if choice==0:#check if user has made a choice
              ...
              if choice==warzone_questions_answers[qnum][6]:#If their choice is right
                score +=1
                scr_label.configure(text=score)
                self.Submit_button.config(text="Submit")
                ...
                score +=0
                scr_label.configure(text="The Correct Answer Was: " + warzone_questions_answers[qnum][5])
